...
XMLSerializer::XMLSerializer( std::ostream& os ) : m_out( os )
{
}
...
void XMLSerializer::setExternalReferences( const std::list<::ecorecpp::parser::Reference>& refs )
{
	...
	const ::ecorecpp::mapping::EList<::ecore::EObject_ptr>::ptr_type& objlist )
{
	...
	for ( const auto obj : *objlist ) {
		if ( m_extendedMetaData ) {
			...
			DEBUG_XMI_MSG( cerr, "name " << m_extendedMetaData->getName( root_eClass ) );
			if ( m_extendedMetaData->isDocumentRoot( root_eClass ) ) {
				DEBUG_XMI_MSG( cerr, "DocumentRoot detected" );
				throw std::logic_error( "Cannot persist DocumentRoot" );
			}
			else
				DEBUG_XMI_MSG( cerr, "DocumentRoot not detected" );
		}
	...
	for ( const auto& element : _packageAliases ) {
		::ecorecpp::mapping::type_definitions::string_t const& ns_uri = element.first->getNsURI();
		::ecorecpp::mapping::type_definitions::stringstream_t root_namespace;
		if ( !m_extendedMetaData || m_extendedMetaData->isQualified( element.first ) ) {
			root_namespace << "xmlns:" << element.second;
...
void XMLSerializer::addUsedPackage( ::ecore::EPackage_ptr package )
{
	auto itAlias = _packageAliases.find( package );
	if ( itAlias != _packageAliases.end() )
		return;

	auto alias = package->getName();

	auto itCounter = _packageNameCounter.find( alias );
	if ( itCounter == _packageNameCounter.end() ) {
		std::tie( itCounter, std::ignore ) = _packageNameCounter.insert( { alias, 1 } );
		_packageAliases.insert( { package, alias } );
		return;
	}

	/* The alias is not yet unique.
	 * The worst case is the insertion sequence model, model_1, model. */
	for ( auto index = itCounter->second; index < 1000; ++index ) {
		auto newAlias = alias + "_" + std::to_string( index );
		auto itNewAlias = _packageNameCounter.find( newAlias );
		if ( itNewAlias == _packageNameCounter.end() ) {
			itCounter->second = index + 1;
			_packageAliases.insert( { package, newAlias } );
			++_packageNameCounter[newAlias];
		}
	}
}

::ecorecpp::mapping::type_definitions::string_t XMLSerializer::get_type( EObject_ptr obj ) const
{
	EClass_ptr cl = obj->eClass();
...
::ecorecpp::mapping::type_definitions::string_t XMLSerializer::get_reference(
	::ecore::EObject_ptr from, ::ecore::EObject_ptr to, bool crossDocument ) const
{
	...
	if ( !fromResource ) {
		const std::string msg{
			...
			+ from->eClass()->getName() + "' and destination EObject of type '"
			+ to->eClass()->getName()
			+ "' because the source object is not assigned to any resource." };
		throw ::ecorecpp::exception::ReferenceStringCreationException( msg, from, to,
																	   crossDocument );
	}
...
void XMLSerializer::create_node( ::ecore::EObject_ptr parent_obj, EObject_ptr child_obj,
								 EStructuralFeature_ptr ef )
{
	...
	if ( child_cl != ef->getEType() ) {
		addUsedPackage( child_cl->getEPackage() );
...
void XMLSerializer::create_crossref_node( ::ecore::EObject_ptr parent_obj, EObject_ptr child_obj,
										  EStructuralFeature_ptr ef )
{
	auto nodeName = ef->getName();
	if ( m_extendedMetaData )
...
void XMLSerializer::serialize_node_attributes( EObject_ptr obj )
{
	 ...
	 *
	 */
	::ecorecpp::mapping::EList<EAttribute_ptr>& attributes = cl->getEAllAttributes();
	...
	for ( auto const& current_at : attributes ) {
		...
		DEBUG_XMI_MSG( cerr, indent << attributeName << " (from " << current_at->getName() << ")" );

		EDataType_ptr atc = as<EDataType>( at_classifier );
		if ( atc ) {
			EFactory_ptr fac = at_classifier->getEPackage()->getEFactoryInstance();
			ecorecpp::mapping::any any = obj->eGet( current_at );

			::ecorecpp::mapping::type_definitions::string_t value =
				fac->convertToString( atc, any );

			DEBUG_XMI_MSG( cerr, indent << attributeName << value );

			if ( ( !value.empty() && value != current_at->getDefaultValueLiteral() )
				 || ( m_keepDefault && value == current_at->getDefaultValueLiteral() ) ) {
	 ...
	 */
	::ecorecpp::mapping::EList<EReference_ptr>& references = cl->getEAllReferences();

	std::vector<std::array<::ecorecpp::mapping::type_definitions::string_t, 3>> crossReferences;

	for ( auto const& current_ref : references ) {
		if ( current_ref->isTransient() || current_ref->isDerived() || current_ref->isContainment()
		...
		DEBUG_XMI_MSG( cerr,
					   indent << referenceName << " (from " << current_ref->getName() << ")" );
		...
		if ( current_ref->getUpperBound() != 1 ) {
			...
			const bool isCrossDocumentReference = std::any_of(
				children->begin(), children->end(), [obj]( ::ecore::EObject_ptr child ) {
			...
			if ( child ) {
				const bool isCrossDocumentReference = obj->eResource() != child->eResource();
	...
	for ( auto it = range.first; it != range.second; ++it ) {
		...
		try {
			auto eref =
				::ecore::as<::ecore::EReference>( getEStructuralFeature( cl, ref._featureName ) );
	...
	for ( auto const& current_at : attributes ) {
		if ( current_at->isTransient() || current_at->isDerived()
			 || current_at->getUpperBound() == 1 || !obj->eIsSet( current_at ) )
			continue;
...
{
	::ecorecpp::mapping::EList<EReference_ptr>& references = obj->eClass()->getEAllReferences();
		...
		for ( auto it = range.first; it != range.second; ++it ) {
			...
			try {
				auto eref = ::ecore::as<::ecore::EReference>(
					getEStructuralFeature( obj->eClass(), ref._featureName ) );
...
::ecore::EStructuralFeature_ptr XMLSerializer::getEStructuralFeature(
